...
Vue.component('products', {
  ...
  "<div class='image' @click='viewProduct(product)' v-bind:style='{ backgroundImage: \"url(\" + product.thumbnailUrl + \")\" }' style='background-size: cover; background-position: center;'></div>" +
  ...
  '<div class="modal-wrapper"> ' +
    ...
    "<i class='close fa fa-times' @click='hideModal()'></i>" +
     ' <div class="modal-header">' +
      ...
      "<div class='imageWrapper'><div class='image' v-bind:style='{ backgroundImage: \"url(\" + modalData.thumbnailUrl + \")\" }' style='background-size: cover; background-position: center;' v-on:mouseover='imageMouseOver($event)' v-on:mousemove='imageMouseMove($event)' v-on:mouseout='imageMouseOut($event)'></div></div>" +
      
      "<h5 class='name'><b>{{ modalData.title }}</b></h5>" +
      "<div class='description'><i>{{ modalData.shortDescription }}</i></div>" +
      "<div class='description'><b>Author: </b> {{ modalData.author }}</div>" +
      "<div class='description'><b>Category: </b>{{ modalData.category }}</div>" +
      "<div class='description'><b>Page Count: </b>{{ modalData.pageCount }}</div>" +
      "<div class='description'><b>ISBN: </b>{{ modalData.isbn }}</div>" +
      "<h5 class='price'>{{ modalData.price | currency }}</h5>" +
      "<label for='modalAmount'>QTY</label>" +
      "<input id='modalAmount' value='{{ modal.title }}' v-model='modalAmount' class='amount' @keyup.enter='modalAddToCart(modalData), hideModal()'/>" +
      "<button @click='modalAddToCart(modalData), hideModal()'>Add to Cart</button>" +
      "</div></div>"+

      '<div class="modal-footer"> </div></div></div></div>'
/*"<div class='modalWrapper' transition='modal' v-show='showModal'>" +
  ...
  props: ['productsData', 'ourProducts', 'cart', 'tax', 'cartSubTotal', 'cartTotal'],
...
Vue.component('cart', {
  ...
  '<td class="align-left"><div class="cartImage" @click="removeProduct(product)" v-bind:style="{ backgroundImage: \'url(\' + product.thumbnailUrl + \')\' }" style="background-size: cover; background-position: center;"><i class="close fa fa-times"></i></div></td>' +
...
var vue = new Vue({
  ...
  data: {
	ourProducts : [{"title":"Unlocking Android"}],
    productsData: [],
    checkoutBool: false,
    cart: [],
    cartSubTotal: 0,
  ...
  created: function () {
	  axios
      .get('http://localhost:8080/ecommerce/books/')
